...
import {handle1155Nfts, handle1155Transfer, handleNetwork} from "../../utils/utilHandlers";
...
export async function handleERC1155batch(
    ...
    const instance = Erc1155__factory.connect(event.address, new FrontierEthProvider());
    let isERC1155 = false;
    ...
    if (!collection) {
        ...
        assert(event.transactionHash, "missing transactionHash")
